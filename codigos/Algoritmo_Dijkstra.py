...
escolas_df = pd.read_csv(rf'{project_root}\dados\Escolas_com_vagas_gps - Copia.csv')
escolas_df = escolas_df.iloc[4:8]
escolas_df.loc[escolas_df.index[:4], 'Vagas'] = [10, 10, 10, 10]
escolas_df.to_csv(rf'{project_root}\dados\micro_Escolas_com_vagas_gps.csv', index=False)
print(escolas_df)

candidatos_df = pd.read_csv(rf'{project_root}\dados\candidatos_concurso_lat_long.csv')
candidatos_df = candidatos_df.head(40)
candidatos_df.to_csv(rf'{project_root}\dados\micro_candidatos_concurso_lat_long.csv', index=False)
...
alocacoes_df.to_csv(rf'{project_root}\dados\alocacao_candidatos_escolas.csv', index=False)

# Exibir a distância total
print(r"Alocações registradas em 'dados\alocacao_candidatos_escolas.csv'")
